...
class DispatchExecutor:
    ...
    def is_stale(self, battery_id: UUID) -> bool:
        """Returns True if the battery hasn't responded within the safe state timeout."""
        last = self._last_contact.get(battery_id)
        if last is None:
            return True
        elapsed = (datetime.now(timezone.utc) - last).total_seconds()
        return elapsed > SAFE_STATE_TIMEOUT_SECONDS

    async def send_safe_state(self, battery_id: UUID) -> None:
        """Emergency: set battery to 0 kW output."""
