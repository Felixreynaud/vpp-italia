...
export function Portfolio() {
  ...
  const refresh = useCallback(async () => {
    try {
      setRefreshing(true);
      setError(null);
      // Portfolio = full catalogue (active + inactive)
  ...
  const handleDelete = async (battery: ConfiguredBattery) => {
    if (!window.confirm(
      `Supprimer définitivement ${battery.name} du portefeuille ?\n\nCela retire la batterie de la VPP, du simulateur, et purge les readings historiques.`
    )) return;
  ...
  const handleToggleActive = async (battery: ConfiguredBattery) => {
    setBusy(battery.battery_id);
    try {
      if (battery.is_active) {
        await deactivateBattery(battery.battery_id);
      } else {
        await activateBattery(battery.battery_id);
      }
      await refresh();
    } catch (err) {
  ...
  const activeCount = batteries.filter((b) => b.is_active).length;
  const inactiveCount = batteries.length - activeCount;

  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Portefeuille batteries</h1>
          <p className="text-sm text-slate-400 mt-1">
            Catalogue complet — création, import depuis FusionSolar, activation/désactivation, suppression.
          </p>
          <div className="flex gap-4 mt-2 text-xs">
            <span className="text-slate-400">Total : <strong className="text-white">{batteries.length}</strong></span>
            <span className="text-success">Actives : <strong>{activeCount}</strong></span>
            <span className="text-slate-500">Inactives : <strong>{inactiveCount}</strong></span>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-400 border-2 border-dashed border-slate-700 rounded-lg">
          Portefeuille vide. Cliquez sur <strong className="text-white">« + Nouvelle batterie »</strong> pour en ajouter.
        </div>
      ) : (
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      >
                        {b.is_active ? (
                          ...
                          <>
                            <Power className="w-3 h-3" /> Activer
                          </>
                        )}
                      </button>
                      <button
                        ...
                        disabled={busy === b.battery_id}
                        className="p-1.5 rounded text-red-400 hover:bg-red-400/10 disabled:opacity-50"
...
// ---------------------------------------------------------------------------
// DiscoverModal (inline) — Huawei FusionSolar bulk import
// ---------------------------------------------------------------------------
...
function DiscoverModal({ onClose, onImported }: DiscoverModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportResult(
        `${res.imported} importée(s) au portefeuille, ${res.skipped} ignorée(s).`
      );
        ...
        <header className="flex items-center justify-between p-4 border-b border-slate-700">
          <div>
            <h2 className="text-lg font-bold text-white">
              Importer depuis Huawei FusionSolar
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Les batteries importées arrivent au portefeuille (inactives).
            </p>
          </div>
