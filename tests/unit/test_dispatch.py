...
from core.dispatch.models import BatterySpec, HourType
...
async def test_gme_client_returns_24_hours() -> None:
    ...
    with (
        patch.object(
            client, "_fetch_mgp", new=AsyncMock(side_effect=RuntimeError("mocked API failure"))
        ),
        patch.object(client, "_fallback_prices", new=AsyncMock(return_value=fake_fallback)),
    ):
...
async def test_gme_client_cache_avoids_double_fetch() -> None:
    ...
    fake_prices = [
        HourlyPrice(hour=h, price_eur_mwh=80.0, zone="SUD", market="MGP") for h in range(24)
    ]
...
async def test_gme_client_pun_index_is_mean() -> None:
    ...
    client._cache[cache_key] = [
        HourlyPrice(hour=h, price_eur_mwh=100.0, zone="SUD", market="MGP") for h in range(24)
    ]
...
async def test_gme_client_zone_prices_independent() -> None:
    ...
    def make_cache(zone: str, price: float) -> list:
        return [
            HourlyPrice(hour=h, price_eur_mwh=price, zone=zone, market="MGP") for h in range(24)
        ]
...
def test_optimize_day_multiple_batteries_independent() -> None:
    ...
    actions_bat1 = [
        hs.actions[bat1.battery_id]
        for hs in schedule.hours.values()
        if bat1.battery_id in hs.actions
    ]
    actions_bat2 = [
        hs.actions[bat2.battery_id]
        for hs in schedule.hours.values()
        if bat2.battery_id in hs.actions
    ]
...
async def test_backtester_report_has_required_fields() -> None:
    with patch.object(
        GMEPriceClient, "get_mgp_prices", new=AsyncMock(return_value=PRICES_HIGH_SPREAD)
    ):
        gme = GMEPriceClient(zone="SUD")
...
async def test_backtester_positive_pnl_high_spread() -> None:
    ...
    assert report.total_pnl_eur > 0, (
        "Expected positive total P&L over 7 days with high price spread"
    )
